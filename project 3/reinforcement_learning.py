...
class RLAgent(Agent[S, A]):
    ...
    def act(self, env: Environment[S, A], observation: S, training: bool = False) -> A:
        ...
        if training and self.should_explore():
            ...
            if not actions:
                return None
            maxval = max(self.compute_q(env, observation, action)
                         for action in actions)
            for action in actions:
                actval = self.compute_q(env, observation, action)
                if(actval == maxval):
                    return action
...
class SARSALearningAgent(RLAgent[S, A]):
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, next_action: Optional[A]):
        ...
        estimated_return = reward + self.discount_factor * \
            self.compute_q(env, next_state, next_action)
        self.Q[str(state)][str(action)] += self.learning_rate * \
            (estimated_return - self.Q[str(state)][str(action)])  # sarsa eqn
...
class QLearningAgent(RLAgent[S, A]):
    ...
    def compute_utility(self, env: Environment[S, A], state: S) -> float:
        ...
        if not self.actions:
            return 0.0
        # returning the max value of the above fn
        return max(self.compute_q(env, state, action) for action in self.actions)
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        estimated_return = reward + self.discount_factor * \
            self.compute_utility(env, next_state)
        self.Q[str(state)][str(action)] += self.learning_rate * \
            (estimated_return - self.Q[str(state)]
             [str(action)])  # the Q euation
...
class ApproximateQLearningAgent(RLAgent[S, A]):
    ...
    # computing q value by multiplying weights value by the corresponding feature value
   ...
   # computing the maximum value that the function above computes
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        features = self.feature_extractor.extract_features(
            env, state)  # features of state
        features_next = self.feature_extractor.extract_features(
            env, next_state)  # features of next state
        if (done == True):
            estimated_return = reward  # features of next state is zero
        else:
            estimated_return = reward + self.discount_factor * \
                self.__compute_utility_from_features(features_next)

        # we somehow must save this value here before putting it in equation
        saved_value = estimated_return - self.compute_q(env, state, action)
        for key in features:
            weights[str(action)][key] += self.learning_rate * \
                saved_value * features[key]  # the appproax Q equation
